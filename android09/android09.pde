...
void mouseReleased(){
  if(NKB.active){
    ...
    if(keySwitch.released(mouseX, mouseY)){
      ...
      if(page_number == 1){
        if(listMaterialSelection.released(mouseX, mouseY-render_offset[page_number])) listMaterial = new KetaiList(this, listMaterialSelection.list);
        else{
          for(int i = 0; i < numData; i++){
  ...
  if(stored_offset[page_number]+mouseY-mouse_position_pressed<0){
    render_offset[page_number] = stored_offset[page_number]+mouseY-mouse_position_pressed;
    stored_offset[page_number] = render_offset[page_number];
  }else{
    render_offset[page_number] = 0;
    stored_offset[page_number] = 0;
  }
...
void mouseDragged(){
  if(stored_offset[page_number] + mouseY - mouse_position_pressed < 0) render_offset[page_number] = stored_offset[page_number] + mouseY - mouse_position_pressed;
