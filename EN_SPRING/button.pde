...
class button{
  ...
  button(int pos_x, int pos_y, int size_x, int size_y, String label){
    ...
    labelText = label;
    labelSize = int(size_y*0.45);
    buttonFont = createFont("MS-PGothic", labelSize);
    buttonPushed = color(50, 10, 50, 200);
    buttonReleased = color(100, 100, 100, 200);
  ...
  void render(){    
    //fill(0, 0, 0);
    if(isButtonPressed){
      //rect(buttonPosition.x+2, buttonPosition.y+2, buttonSize.x-4, buttonSize.y-4);
      fill(buttonPushed);
      rect(buttonPosition.x+5, buttonPosition.y+5, buttonSize.x-10, buttonSize.y-10);
    }else{
    ...
    //textSize(labelSize);
    textFont(buttonFont);
    text(labelText, buttonPosition.x+10, buttonPosition.y+buttonSize.y-10);
  }
