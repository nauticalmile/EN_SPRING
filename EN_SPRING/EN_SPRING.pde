...
void draw(){
  ...
  //----------------------------------------------------------DRAW ELLIPSES TO SEE THE SCREEN UPDATING.
  
  float t = frameCount*PI/180.0f;
  a.x = 100.0f*sin(t)+150.0f;
  a.y = 200.0f*cos(t)+500.0f;
  b.x = 50.0*cos(3.0*t)+a.x;
  b.y = 50.0f*sin(3.0*t)+a.y;
  fill(180, 150, 200);
  ellipse(150, 500, 800, 800);
  fill(150, 0, 200);
  ellipse(a.x, a.y, 20.0f, 20.0f);
  fill(200, 0, 100);
  ellipse(b.x, b.y, 5.0f, 5.0f);
  ...
  if(isWipe){
    ...
    //-------------------------------------------------------------WIPE ACTION WITHOUT ANY USER CONTROL.
      ...
      if(page_number == 0 && wipeOffset < -wipeRange){
        isPageChange = true;
      }else if(page_number == 2 && wipeOffset > wipeRange){
        isPageChange = true;
      }else if(page_number == 1 && abs(wipeOffset) > wipeRange){
        isPageChange = true;
      }else isPageChange = false;
      if(isPageChange){
...
void mouseDragged(){
  ...
  //数値キーボードがアクティブではなく，ワイプ中でもなければ，以下を実行．
